...
CENTML_API_KEY = os.environ.get("CENTML_API_KEY", None)
...
def call_centml(prompt, model_id="deepseek-ai/DeepSeek-V3-0324", system_prompt=DEFAULT_SYSTEM_PROMPT):
    print("=== call_centml ", prompt)
    ...
    print("[INFO] Created CentML client call_centml")
    ...
    print("call_centml resp ", response)
    return response.choices[0].message.content.strip()


def arrange_message_for_centml(item_list):
...
def call_centml_with_messages(messages, model_id="deepseek-ai/DeepSeek-V3-0324", system_prompt=DEFAULT_SYSTEM_PROMPT):
    centml_api_key = os.getenv("CENTML_API_KEY")  # Ensure the CENTML_API_KEY environment variable is set
    if not centml_api_key:
        raise ValueError("CENTML_API_KEY environment variable is not set.")
    ...
    print("=== call_centml with message ", messages)
    response = client.chat.completions.create(
        model=model_id,
    ...
    print("=== call_centml with message out ", response)
    return response.choices[0].message.content.strip()
...
Today is 6/12/2023. Base on the aforementioned webpage, tell me how many fulfilled orders I have over the past month, and the total amount of money I spent over the past month.'''
    print(call_centml(prompt=prompt, model_id="deepseek-ai/DeepSeek-V3-0324"))
